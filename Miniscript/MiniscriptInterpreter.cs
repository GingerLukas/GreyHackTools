...
namespace Miniscript {
	...
	public class Interpreter {
		...
		public void RunUntilDone(double timeLimit=60, bool returnEarly=true,Action<TAC.Machine> stepCallbackAction = null,WaitHandle stepWaitHandle = null) {
			try {
				if (vm == null) {
					Compile();
				...
				while (!vm.done && !vm.yielding) {
					...
					vm.Step(stepCallbackAction,stepWaitHandle);		// update the machine
