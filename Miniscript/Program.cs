...
".Split(sep,StringSplitOptions.None);
		...
		Interpreter repl = new Interpreter(code);
        repl.Compile();
        repl.RunUntilDone(Double.MaxValue,true, m =>
        {
            try
            {
                TAC.Context context = m.stack.Peek();
                Console.WriteLine(context.code[context.lineNum].location.lineNum + " " + code[context.code[context.lineNum].location.lineNum - 1]);
				Console.WriteLine(context.code[context.lineNum]);
                Console.ReadKey();
			}
            catch (Exception e) { }
            handle.Set();
